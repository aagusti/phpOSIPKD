...
class Penelitian extends CI_Controller
{
    ...
    public function index()
    {
		...
		$data['data_source']    = active_module_url() . "penelitian/grid?proses_id=$proses_id";
    ...
    function grid()
    {
        ...
        $aColumns       = array('id', 'transno', 'nop2', 'tanggal', 'wp_nama', 'bayar', 'ppatnm');
        $sIndexColumn   = "transno";
		...
		/* Filtering */
        $sWhere = "WHERE b.is_validated = $proses_id 
        ...
        /* Total Data */
        $sql = "SELECT  COUNT(*) c FROM bphtb_bank b ";
        ...
        if ($search) {
            $sql_query_r = "SELECT  COUNT(*) c FROM bphtb_bank ";
         ...
         * Output
         */
        $sql_query_r = "SELECT  b.*, 
			b.kd_propinsi||'.'||b.kd_dati2||'.'||b.kd_kecamatan||'.'||b.kd_kelurahan||'.'||b.kd_blok||'.'||b.no_urut||'.'||b.kd_jns_op nop2,
			p.nama ppatnm FROM bphtb_bank b
			LEFT JOIN bphtb_validasi v ON b.id=v.bank_id
        ...
        foreach ($qry->result() as $aRow) {
            ...
            for ($i = 0; $i < count($aColumns); $i++) {
                if ($i == 3)
                    $row[] = (strtotime($aRow->tanggal)) ? (string) date('d/m/Y', strtotime($aRow->tanggal)) : '';
                elseif ($i == 5)
    ...
    public function register()
    {
        ...
        $data['current'] = 'penelitian';
        $data['apps']    = $this->apps_model->get_active_only();
        $data['faction'] = active_module_url('penelitian/do_register');
        ...
        if ($id && $bank = $this->bank_model->get($id)) {
			...
			if (!empty($bank->no_tagihan)) {
				$tgh = explode(".", $bank->no_tagihan);
				$data['dt']['tahun']   = $tgh[0];
				$data['dt']['kode']    = $tgh[1];
				$data['dt']['no_sspd'] = $tgh[2];
			} else {
				$data['dt']['tahun']   = '';
				$data['dt']['kode']    = '';
				$data['dt']['no_sspd'] = '';
