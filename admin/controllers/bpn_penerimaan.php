...
class Bpn_penerimaan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['data_source']    = active_module_url() . "bpn_penerimaan/grid/{$proses_id}";
    ...
    function grid()
    {
		...
		$query = $this->penerimaan_model->grid_bpn($status);
		if($query) {
    ...
    public function register()
    {
        ...
        $data['current'] = 'bpn';
        $data['apps']    = $this->apps_model->get_active_only();
        $data['faction'] = active_module_url('bpn_penerimaan/do_register');
        ...
        if ($id && $terima = $this->penerimaan_model->get($id)) {
            ...
            $this->load->view('form_contents/vbpn_penerimaan_form', $data);
        } else {
            show_404();
        }
    }
    ...
    public function do_register()
    {
        ...
        if ($this->form_validation->run() == TRUE) {		
			...
			$validasi_id = $validasi->num_rows() > 0 ? $validasi->row()->id : NULL;
			...
			if($bank = $this->penerimaan_model->get($id)) {
				...
				$this->db->insert('bphtb_bpn',$post_data);
